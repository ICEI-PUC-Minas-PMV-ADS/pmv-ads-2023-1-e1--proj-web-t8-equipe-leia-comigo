...
for ( var i=0; i<reviwesObject.length; i++ ){
    cardsCreate_reviews.insertAdjacentHTML(
        "beforeend",
        `<div class="row__posters" id="${reviwesObject[i].id}"></div>`
    );
}
...
for (var i = 0; i < reviwesObject.length; i++) {
    const cards_reviews = document.getElementById(reviwesObject[i].id);
    const html_reviews = `<img class="img-${reviwesObject[i].id} slider-img" src="${reviwesObject[i].image}" />
    <h2 class="title_description">${reviwesObject[i].description}</h2>`;
